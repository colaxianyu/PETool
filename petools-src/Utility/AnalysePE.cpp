...
namespace petools {
	...
	void AnalysePE::init(const std::byte* begin, DWORD file_size) {
		file_buffer_.resize(file_size);
		memcpy(file_buffer_.data(), begin, file_size);
		fileBufferSize_ = file_size;
		SetHeaders();
	}
	...
	bool AnalysePE::IsPEFile() {
		if (*(WORD*)file_buffer_.data() != IMAGE_DOS_SIGNATURE) {
			return false;
		}
		IMAGE_DOS_HEADER* tempDosHeader = (IMAGE_DOS_HEADER*)file_buffer_.data();
	...
	bool AnalysePE::SetHeaders() {
		...
		headers_.dosHeader = (IMAGE_DOS_HEADER*)file_buffer_.data();
	...
	void DisplayResource(IMAGE_RESOURCE_DIRECTORY_ENTRY* resourceEntry) {
		IMAGE_RESOURCE_DIRECTORY* resourceTable = AnalysePE::instance().GetResource();
	...
	void GetAppName() {
		...
		char* index = (char*)((DWORD)AnalysePE::instance().GetHeaders().dosHeader
			+ AnalysePE::instance().RVAToFOA(stringData->OffsetToData));
...
import++;
	...
	//	memcpy(tempBuffer, file_buffer_[0], fileBufferSize_);
	...
	//	memcpy(newFileBuffer, file_buffer_[0], haveInfo ? fileBufferSize_ - infoSize : fileBufferSize_);
	...
	//			memcpy(newFileBuffer, file_buffer_[0], newHeaderSize);
	//			memcpy(newFileBuffer + newHeaderSize, file_buffer_[0] + headers_.optionalHeader->SizeOfHeaders
	...
	//	memcpy_s(newFileBuffer, newBufferSize, file_buffer_[0], fileBufferSize_);
